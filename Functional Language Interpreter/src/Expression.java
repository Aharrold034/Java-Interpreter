...
public class Expression {
    ...
    public DataValue evaluate() throws Exception {
        if (this.op == null) {
         ...
         // Adding the '#' operator to the expression class
            else if (op.toString().equals("#")) {
            	if (rhs.getType() == DataValue.Type.STRING_VALUE) {
            		String str1 = rhs.toString();
                    str1 = str1.substring(1, str1.length()-1);
                    return new IntegerValue(str1.length());
            	}
            }
            ...
            if (lhs.getType() == rhs.getType()) {
                if (op.toString().equals("==")) {
                    ...
                    //added section for the removal of '"' on strings if necessary
                    ...
                    if (op.toString().equals("+")) {
                        ...
                        return new StringValue(str1.substring(0, str1.length())
                                + str2);
                        }
                 ...
                 // Adding the '@' operator to the expression class (if lhs is an int)
                    else if (op.toString().equals("@")) {
                       String str1 = lhs.toString();
                       return new StringValue(valueAt(str1, num2));
                    }
            ...
            // Adding the '@' operator to the expression class
            	...
            	//add error if not string or int value
                String str1 = lhs.toString();
                str1 = str1.substring(1, str1.length()-1);
                int num2 = ((Integer) (rhs.getValue()));
                return new StringValue(valueAt(str1, num2));
    ...
    /**
     * Attains the value at a certain index of an input.
     *   @return the value at the requested index.
     */
    private String valueAt(String str1, int num2) throws Exception {
    	if (str1.length() - 1 < num2)
            throw new Exception("SYNTAX ERROR: String is too short to access the value at index " + num2);
    	else if (num2 < 0)
    		throw new Exception("SYNTAX ERROR: Cannot access a negative index value" + num2);
        else {
        	return (str1.substring(num2, num2 + 1));
        }
    }
